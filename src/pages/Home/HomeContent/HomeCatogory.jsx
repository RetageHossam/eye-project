...
import { getCatgory } from "../HomeCatgoryFetch";
  export default function HomeCatogory() {
    const [catagory,setCatagory]=useState([]);
    useEffect(()=>{
        getCatgory(setCatagory);
    },[])
 return (
  <section className="py-24 bg-[#111111] relative border-y border-[#262626]">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
     ...
     <div className=" justify-center flex items-center ">
         <span className="text-amber-600 text-small w-[10%] justify-center flex items-center   bg-transparent border-amber-600 rounded-3xl border-2 px-4 py-2.5 ">
        ...
        <h2 className="text-5xl my-3 font-bold text-white">استكشف حسب الموضوع</h2>
        <p className="section-subtitle max-w-lg mx-auto text-white">
          اعثر على محتوى مصمم حسب اهتماماتك
        </p>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 ">
         {catagory.map((item)=>{
        return <a className="group relative block p-6 rounded-2xl bg-[#161616] border border-[#262626] overflow-hidden hover:border-orange-500/30 transition-all duration-500 hover:-translate-y-1" href="/blog?category=إضاءة" data-discover="true">
          ...
          <div className="relative z-10">
            <div className="w-12 h-12 bg-orange-500/10 rounded-xl flex items-center justify-center mb-4 group-hover:bg-white/20 transition-colors duration-300 border border-orange-500/20 group-hover:border-transparent">
               <i className={`fa-solid ${item.icon} text-xl text-orange-500 group-hover:text-white transition-colors duration-300`} />
            </div>
